...
$total = 'select count(*) as total from ' . $wpdb->prefix . 'application_data where status = "modified"  and deleted_on is null;';
$resulttotal = $wpdb->get_results($total);
$resulttotal = $resulttotal[0]->total;
if (isset($_GET['paged'])) {
    $sql = "select * from " . $wpdb->prefix . "application_data where status = 'modified' and deleted_on is null limit " . $start_from . "," . $per_page;
} else {
    $sql = "select * from " . $wpdb->prefix . "application_data where status = 'modified' and deleted_on is null limit " . $start_from . "," . $per_page;
}
                ...
                <td>
                    ...
                    foreach ($roles as $role) {
                        ...
                        if (array_key_exists("staff", $role)) {
                            if(!empty($val->crm_id)) {
                                $approve_link =  get_admin_url()."admin-post.php?action=approve-application-form&id=".base64_encode($val->id)."&is_update=1";
                            }
                            else{
                                $approve_link =  get_admin_url()."admin-post.php?action=approve-application-form&id=".base64_encode($val->id);
                            }
                            
                            ?>
                            <a href="<?php echo site_url(); ?>/index.php/listings/?page=create-form-register&id=<?php echo base64_encode($val->id); ?>&view=true">View</a>&nbsp;&nbsp;
                            <a href="<?php echo $approve_link ;?>" class="" value="">Approve </a>
                        <?php } else { ?>
                            <a href="<?php echo site_url(); ?>/index.php/listings/?page=create-form-register&id=<?php echo base64_encode($val->id); ?>">Edit</a>&nbsp;&nbsp;
                            <a href="#app_diff" class="colorbox-inline" onclick="getAppDiff('<?php echo $val->id; ?>','app','<?php echo $company_name;?>');getAppDocDiff('<?php echo $val->id; ?>','doc','<?php echo $company_name;?>')">Diff</a>&nbsp;&nbsp;
                            <a href="<?php echo site_url(); ?>/index.php/listings/?page=create-form-register&id=<?php echo base64_encode($val->id); ?>&view=true">View</a>
                            <?php
                        }
                    }
